...
export const ChatMember = ({member}: { member: User }) => <View style={ListViewStyle.container}>
  <Text style={ListViewStyle.title}>{member.name}</Text>
  <Text style={ListViewStyle.subtitle}>last seen recently</Text>
</View>

export const ManagedChatMember = ({member, buttonText, onPress}: { member: User, buttonText: string, onPress: () => void }) =>
  <View style={[ListViewStyle.container, {flexDirection: 'row', justifyContent: 'space-between'}]}>
  <View style={{flexDirection: 'column'}}>
    <Text style={ListViewStyle.title}>{member.name}</Text>
